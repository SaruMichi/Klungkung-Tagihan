...
public class CtrlAppPriv {
    ...
    public int action(int cmd, long appPrivOID, int start, int vectSize, int recordToGet, HttpServletRequest request) {
        msgString = "";
        int excCode = 0;
        switch(cmd){
            case Command.ADD :
            ...
            case Command.SAVE :
                ...
                long rsCode = 0;
                if (appPriv.getOID() == 0){
		    String getDate = FRMQueryString.requestString(request, FrmAppPriv.fieldNames[FrmAppPriv.FRM_REG_DATE]);
		    appPriv.setRegDate(Formater.formatDate(getDate, "dd MMM yyyy"));
