 ...
 * @author Ardiadi
 */
public class SessUserWPSession {
    ...
    private AppUserWP appUserWp = new AppUserWP();
   ...
   // private Hashtable<String, Boolean> appG1G2Obj = new Hashtable();
    ...
    public SessUserWPSession() {
        appUserWp.setUserStatus(AppUserWP.STATUS_LOGOUT);
    }

    public SessUserWPSession(String hostIP) {
        appUserWp.setUserStatus(AppUserWP.STATUS_LOGOUT);
        appUserWp.setLastLoginIp(hostIP);
    }

    public AppUserWP getAppUser(){
    	return this.appUserWp;
    }

    public boolean isLoggedIn(){
        if( /*(this.appUser!=null) && (this.appUser.getOID()!=0) && */
             (this.appUserWp.getUserStatus()==AppUserWP.STATUS_LOGIN))
            return true;
        else
            return false;
    }
   ...
   /*public boolean checkPrivilege(int objCode){
       if(!isLoggedIn())
           return false;
       return SessAppWpPrivilegeObj.existCode(this.privObj, objCode);
   }
   ...
   public boolean checkPrivG1G2(int privG1, int privG2){
    ...
    public int doLogin(String loginID, String password, String noTelp){
        AppUserWP user = PstAppUserWP.getByLoginIDAndPassword(loginID, password, noTelp);

        if(user==null)
            return DO_LOGIN_SYSTEM_FAIL;

        if(user.getWpUserId().length()==0)
            return DO_LOGIN_NOT_VALID;

        user.setLastLoginIp(this.appUserWp.getLastLoginIp());
        user.setUserStatus(AppUserWP.STATUS_LOGIN);
        ...
        this.appUserWp = user;
        /*privObj = PstAppUserWP.getUserPrivObj(this.appUser.getOID());
        ...
        System.out.println(" User login OK status ->" + appUserWp.getUserStatus());
        return DO_LOGIN_OK;
    }
     ...
     */
    public int doApproval(String loginID, String password, String noTelp){
        AppUserWP user = PstAppUserWP.getByLoginIDAndPassword(loginID, password, noTelp);
    ...
    public void doLogout(){
        if((this.appUserWp!=null) && (this.appUserWp.getUserStatus() == appUserWp.STATUS_LOGIN)){
        }
    }
    ...
    public void printAppUser(){
        ...
        System.out.println(" user ID = "+ this.appUserWp.getOID());
        System.out.println(" login ID = "+ this.appUserWp.getWpUserId());
        System.out.println(" employee = "+ this.appUserWp.getWpUserNama());
        System.out.println(" status = "+ this.appUserWp.getUserStatus());
    }
}
