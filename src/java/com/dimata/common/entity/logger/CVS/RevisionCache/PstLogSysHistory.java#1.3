...
public class PstLogSysHistory extends DBHandler 
    ...
    public static final int FLD_LOG_QUERY_ROLEBACK=11;
   ...
   //NAMA MENGIKUTI NAMA2 FIELD DI DATABASE
    ...
    try {
            ...
            pstLogSysHistory.setString(FLD_LOG_QUERY_ROLEBACK, logSysHistory.getQueryRoleBack());
 ...
 */
    ...
    public static Vector listDocType(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+fieldNames[FLD_LOG_DOCUMENT_TYPE]+" FROM " + TBL_LOG_HISTORY;
            ...
            {
                LogSysHistory logSysHistory = new LogSysHistory();
                logSysHistory.setLogDocumentType(rs.getString(fieldNames[FLD_LOG_DOCUMENT_TYPE]));
                lists.add(logSysHistory);
            }
                rs.close();
                return lists;
     ...
     public static void resultToObject(ResultSet rs, LogSysHistory logSysHistory) {
        ...
        try {
           ...
           // logSysHistory.setLogOpenUrl(rs.getString(PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_OPEN_URL]));
            logSysHistory.setLogUpdateDate(rs.getDate(PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_UPDATE_DATE]));
            logSysHistory.setLogDetail(rs.getString(PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_DETAIL]));
            logSysHistory.setQueryRoleBack(rs.getString(PstLogSysHistory.fieldNames[PstLogSysHistory.FLD_LOG_QUERY_ROLEBACK]));
    ...
    */ 
    public static long getFirstUserId(long docId){
        long userId =0;
        com.dimata.common.db.DBResultSet dbrs = null;
        try {
            ...
            dbrs = com.dimata.common.db.DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static int deleteHistorySegment(long periodeId, String cif) {
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "DELETE FROM " + TBL_LOG_HISTORY
                + " WHERE " + fieldNames[FLD_LOG_DOCUMENT_NUMBER] + " = '" + periodeId+"'";
        if(!cif.equals("")){
            stSql=stSql+" AND "+fieldNames[FLD_LOG_OPEN_URL]+" like '%"+cif+"%' ";
        }
        try {
