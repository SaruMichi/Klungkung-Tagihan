...
public class PstAppUser extends DBHandler implements I_DBInterface, I_DBType, I_Persintent  {
    ...
    public static final String TBL_APP_USER = "slik_app_user";
    ...
    public static final int FLD_KD_CABANG = 15;
    public static  final String[] fieldNames =
    {
        ...
        ,"EMPLOYEE_ID","USER_GROUP", "START_TIME", "END_TIME", "KD_CABANG" // END TIME , TIME IN USE ONLY
    } ;
    ...
    public static AppUser fetch(long oid) {
        ...
        try {
            ...
            entObj.setKdCabangBank(pstObj.getString(FLD_KD_CABANG));
        }
        catch(DBException e) {
    ...
    public static long insert(AppUser entObj) throws DBException, com.dimata.common.db.DBException  {
        ...
        pstObj.setString(FLD_KD_CABANG, entObj.getKdCabangBank());
  ...
  */
    ...
    private static void resultToObject(ResultSet rs, AppUser entObj) {
        try {
            ...
            entObj.setKdCabangBank(rs.getString(fieldNames[FLD_KD_CABANG]));
        }catch(Exception e){
...
//    }
    ...
    public static AppUser getByLoginID(String loginID, String password) {
        if((loginID==null) || (loginID.length()<1) || (password==null) || (password.length()<1))
        return null;

        try{

            String whereClause = " "+fieldNames[FLD_LOGIN_ID]+"='"+loginID.trim()+"'";
    ...
    public static String checkPass(String userId)
    {
        DBResultSet dbrs = null;
        String result = "";
        try
        {
            String sql = "SELECT * FROM " + TBL_APP_USER + " WHERE " +
                PstAppUser.fieldNames[PstAppUser.FLD_LOGIN_ID] + " = '" + userId+"'";
    ...
    //--------------- add by edhy ---------------------
    public static long checkUser(String kode) {
        DBResultSet dbrs = null;
        long result = 0;
        try {
            String sql = "SELECT * FROM " + TBL_APP_USER+ " WHERE "
                    + ""+PstAppUser.fieldNames[PstAppUser.FLD_LOGIN_ID]+"='"+kode+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
    ...
    public static long updateCabangBank(String cabang, long userId) throws DBException {
        int iResult = 0;
        if(userId!=0){
            DBResultSet dbrs = null;
            String stSql = "UPDATE slik_app_user SET KD_CABANG='"+cabang+"' WHERE USER_ID='"+userId+"';";
            try {
