    ...
    implements Runnable
{
    ...
    private String[] connCallerObject; //object yang memanggil koneksi
    ...
    throws IOException
    {
        ...
        connCallerObject =  new String[maxConn];
...
//        SimpleDateFormat simpledateformat = new SimpleDateFormat("yyyy.MM.dd G 'at' hh:mm:ss a zzz");
    ...
    public void run()
    {
            ...
            for(int j = 0; j < currConnections; j++)
            {
                ...
                catch(SQLException _ex)
                {
                    ...
                    catch(SQLException sqlexception1)
                    {
                        ...
                        log.println("Failed: " + ex.toString());
                        connStatus[j] = 0;
                    }
                }
                finally
        ...
        System.out.print("Keluar dari thread");
    }
    ...
    public Connection getConnection(String callerObjectName)
    {
                    ...
                    do
                        synchronized(connStatus)
                        {
                            if(connStatus[k] < 1 && !connPool[k].isClosed())
                            {
                                ...
                                connLast = k;
                                this.connCallerObject[k]=callerObjectName;
                                flag = true;
                                break;
                            }
                            j++;
                ...
                synchronized(this)
                {
                    if(currConnections < maxConns)
                        ...
                        catch(SQLException sqlexception)
                        {
                            ...
                            log.println("Unable to create new connection: " + ex.toString());
                        }
                }
                try
                {
    ...
    private void createConn(int i) throws SQLException,Exception{
        try{
            createConn (i, "");
        }catch(SQLException ex){
            ex.printStackTrace(System.err);
            throw new SQLException("Create Connection Eror", ""+ex.toString());
        }catch(Exception exx){
            throw new Exception("Create Connection Eror"+exx.toString());
        }
    }
    private void createConn(int i, String callerObject)
        throws SQLException,ClassNotFoundException,Exception
    {
        ...
        try
        {
            ...
            connCallerObject[i]=callerObject;
        }
        catch(ClassNotFoundException _ex) {
            throw new ClassNotFoundException("createConn "+_ex.toString());
        }
        catch(Exception e) {
            throw new Exception("createConn "+e.toString());
        }
