...
<%
    ...
    Vector regionKey = new Vector(1,1);
    Vector regionVal = new Vector(1,1);
    
    if(regionList.size() > 0){
	for(int i = 0; i < regionList.size(); i++){
	    Kabupaten kabupaten = (Kabupaten) regionList.get(i);
	    regionKey.add(""+kabupaten.getOID());
	    regionVal.add(""+kabupaten.getNmKabupaten());
	}
    }
%>
<html>
	...
	<input type="hidden" id="command" value="<%= Command.NONE %>">
		...
		<nav class="navbar navbar-static-top">
			...
			<div class="collapse navbar-collapse" id="navbar-collapse">
				...
				<li><a href="javascript:" id="filter"><i class="fa fa-filter"></i> Set Chanel & Location</a></li>
				<li><a href="javascript:" class="addreport" data-for="addreport" data-oid="0" data-content-id="0" data-linked-id="0"><i class="fa fa-plus"></i> Add Report</a></li>
				<li class="dropdown">
				    <a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-info-circle"></i> Information <span class="caret"></span></a>
	...
	<script type="text/javascript">
		...
		setContent = function(){
			...
			showMarker();
                    ...
                    map.addListener("idle", function(event){
                        setTimeout(function(){
                            getDataCurrentPosition("#curentLocation");
		    ...
		    map.controls[google.maps.ControlPosition.RIGHT_BOTTOM].push(document.getElementById('getyourposition'));
                    ...
                    //PLACE CURENT LOCATION ON THE SELECTOR TAG,KOYO
		...
		addEditReport = function(elementId){
			...
			getDataCurrentPosition("#yourposition");
			dataFor = $(this).data('for');
			$("#oidreport").val(oid);
			$("#reportdatafor").val(dataFor);
			var getYourPosition = $("#yourposition").val();
			...
			dataSend = {
			    ...
			    "FRM_FIELD_LOCATION" : getYourPosition,
		...
		$("form#reportform").submit(function(){
		    getDataCurrentPosition("#<%= FrmReport.fieldNames[FrmReport.FRM_FIELD_REPORT_POST_LOCATION] %>");
			...
			<input type="hidden" id="<%= FrmReport.fieldNames[FrmReport.FRM_FIELD_REPORT_POST_LOCATION] %>" name="<%= FrmReport.fieldNames[FrmReport.FRM_FIELD_REPORT_POST_LOCATION] %>">
					...
					<div class="col-md-6">
						...
						<div class="form-group">
						    ...
						    <div class="col-md-10">
							<%= ControlCombo.draw(FrmKabupaten.fieldNames[FrmKabupaten.FRM_FIELD_ID_KABUPATEN], "-- Select Region", "", regionKey, regionVal, "data-for='getkecamatan' data-replace-to='#"+FrmKecamatan.fieldNames[FrmKecamatan.FRM_FIELD_ID_KECAMATAN]+"'", "form-control controlcombo") %>
						    </div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<select id="<%= FrmDusun.fieldNames[FrmDusun.FRM_FIELD_ID_DUSUN] %>" name="<%= FrmDusun.fieldNames[FrmDusun.FRM_FIELD_ID_DUSUN] %>" class="form-control">
							    <option value=""> -- Select Sub-village --</option>
							</select>
						    </div>
						</div>
					    </div>
	...
	<input type="hidden" id="yourposition">
		...
		<div class="col-md-12">
                    ...
                    <div class="form-group" style="max-height: 500px; overflow-x: hidden;">
