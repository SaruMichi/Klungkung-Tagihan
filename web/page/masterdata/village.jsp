...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | VILLAGE</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showvillageform">
			    <i class="fa fa-plus"></i> Add Village
			</button>
			<button class="btn btn-danger btndeleteareatype" data-for="village">
	...
	$(document).ready(function(){
		...
		$(elementId).click(function(){
		    ...
		    //SET TITLE MODAL
		    if(oid != 0){
			if(dataFor == 'showvillageform'){
			    $(".addeditgeneral-title").html("Edit Village");
			}

		    }else{
			if(dataFor == 'showvillageform'){
			    $(".addeditgeneral-title").html("Add Village");
			}
		    }
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxVillage", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxVillage", null, true, "json");
	    ...
	    //FORM HANDLER
	    chanelForm = function(){
		validateOptions("#<%= FrmDesa.fieldNames[FrmDesa.FRM_FIELD_ID_KECAM] %>", 'text', 'has-error', 1, null);
                validateOptions("#<%= FrmDesa.fieldNames[FrmDesa.FRM_FIELD_NAMA_DESA] %>", 'text', 'has-error', 1, null);
                validateOptions("#<%= FrmDesa.fieldNames[FrmDesa.FRM_FIELD_KD_DESA] %>", 'text', 'has-error', 1, null);
	    }
		...
		dataTablesOptions("#villageElement", "tableVillageElement", "AjaxVillage", "listvillage", callBackDataTables);
	    }
	    ...
	    deletegeneral(".btndeleteareatype", ".<%= FrmDesa.fieldNames[FrmDesa.FRM_FIELD_ID_DESA]%>");
		...
		if(generaldatafor == "showvillageform"){
		    chanelForm();
		    onDone = function(data){
			runDataTables();
		    };
