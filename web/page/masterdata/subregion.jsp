...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | SUB REGION</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showsubregionform">
			    <i class="fa fa-plus"></i> Add Sub Region
			</button>
			<button class="btn btn-danger btndeleteareatype" data-for="subregion">
	...
	$(document).ready(function(){
	    ...
	    activeMenu("#masterdata", "#subregion");
		...
		$(elementId).click(function(){
		    ...
		    //SET TITLE MODAL
		    if(oid != 0){
			if(dataFor == 'showsubregionform'){
			    $(".addeditgeneral-title").html("Edit Sub Region");
			}

		    }else{
			if(dataFor == 'showsubregionform'){
			    $(".addeditgeneral-title").html("Add Sub Region");
			}
		    }
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxKecamatan", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    if(dataFor == "subregion"){
				onDone = function(data){
				    runDataTables();
				};
			    }
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxKecamatan", null, true, "json");
	    ...
	    //FORM HANDLER
	    chanelForm = function(){
		validateOptions("#<%= FrmKecamatan.fieldNames[FrmKecamatan.FRM_FIELD_ID_KABUPATEN] %>", 'text', 'has-error', 1, null);
                validateOptions("#<%= FrmKecamatan.fieldNames[FrmKecamatan.FRM_FIELD_KD_KECAMATAN] %>", 'text', 'has-error', 1, null);
                validateOptions("#<%= FrmKecamatan.fieldNames[FrmKecamatan.FRM_FIELD_NM_KECAMATAN] %>", 'text', 'has-error', 1, null);
	    }
		...
		dataTablesOptions("#subRegionElement", "tableSubRegionElement", "AjaxKecamatan", "listsubregion", callBackDataTables);
	    }
	    ...
	    deletegeneral(".btndeleteareatype", ".<%= FrmKecamatan.fieldNames[FrmKecamatan.FRM_FIELD_ID_KECAMATAN]%>");
		...
		if(generaldatafor == "showsubregionform"){
		    chanelForm();
		    onDone = function(data){
			runDataTables();
		    };
