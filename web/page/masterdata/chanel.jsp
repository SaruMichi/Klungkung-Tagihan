...
<%@page import="com.dimata.balismartisland.form.FrmChanelType"%>
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | CHANEL</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showchanelform">
			    <i class="fa fa-plus"></i> Add Chanel
			</button>
			<button class="btn btn-danger btndeleteareatype" data-for="chanel">
	...
	$(document).ready(function(){
		...
		$(elementId).click(function(){
		    ...
		    //SET TITLE MODAL
		    if(oid != 0){
			if(dataFor == 'showchanelform'){
			    $(".addeditgeneral-title").html("Edit Chanel");
			}

		    }else{
			if(dataFor == 'showchanelform'){
			    $(".addeditgeneral-title").html("Add Chanel");
			}
		    }
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxChanel", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    if(dataFor == "chaneltype"){
				onDone = function(data){
				    runDataTables();
				};
			    }
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxChanel", null, true, "json");
	    ...
	    //FORM HANDLER
	    chanelForm = function(){
		validateOptions("#<%= FrmChanel.fieldNames[FrmChanel.FRM_FIELD_CHANEL_NAME] %>", 'text', 'has-error', 1, null);
		validateOptions("#<%= FrmChanel.fieldNames[FrmChanel.FRM_FIELD_CHANEL_TYPE_ID] %>", 'text', 'has-error', 1, null);
	    }
		...
		dataTablesOptions("#chanelElement", "tableChanelElement", "AjaxChanel", "listchanel", callBackDataTables);
	    }
		...
		$(elementId).click(function(){
		    ...
		    oid = $(this).data('oid');
                    $("#formupload").attr("action","<%=approot%>/page/masterdata/uploadchanelimage.jsp?FRM_FIELD_OID="+oid+"&approot=<%=approot%>");
		    dataFor = $(this).data('for');
                    $("#tempname").val("");
		    $("#generaldatafor").val(dataFor);
		    $("#oiddata").val(oid);
	    ...
	    deletegeneral(".btndeleteareatype", ".<%= FrmChanel.fieldNames[FrmChanel.FRM_FIELD_CHANEL_ID]%>");
		...
		if(generaldatafor == "showchanelform"){
		    chanelForm();
		    onDone = function(data){
			runDataTables();
		    };
