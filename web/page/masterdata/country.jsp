...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | COUNTRY</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showcountryform">
	...
	$(document).ready(function(){
		...
		$(elementId).click(function(){
		    ...
		    //SET TITLE MODAL
		    if(oid != 0){
			if(dataFor == 'showcountryform'){
			    $(".addeditgeneral-title").html("Edit Country");
			}

		    }else{
			if(dataFor == 'showcountryform'){
			    $(".addeditgeneral-title").html("Add Country");
			}
		    }
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxNegara", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxNegara", null, true, "json");
	    ...
	    //FORM HANDLER
	    chanelForm = function(){
		validateOptions("#<%= FrmNegara.fieldNames[FrmNegara.FRM_FIELD_BENUA] %>", 'text', 'has-error', 1, null);
		validateOptions("#<%= FrmNegara.fieldNames[FrmNegara.FRM_FIELD_NM_NEGARA] %>", 'text', 'has-error', 1, null);
	    }
		...
		dataTablesOptions("#countryElement", "tableCountryElement", "AjaxNegara", "listcountry", callBackDataTables);
	    }
	    ...
	    deletegeneral(".btndeleteareatype", ".<%= FrmNegara.fieldNames[FrmNegara.FRM_FIELD_ID_NEGARA]%>");
		...
		if(generaldatafor == "showcountryform"){
		    chanelForm();
		    onDone = function(data){
			runDataTables();
		    };
