<%@page import="com.dimata.harisma.form.masterdata.FrmPropinsi"%>
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | PROVINCE</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showprovinceform">
			    <i class="fa fa-plus"></i> Add Province
			</button>
			<button class="btn btn-danger btndeleteProvince" data-for="province">
	...
	$(document).ready(function(){
	    ...
	    activeMenu("#masterdata", "#province");
		...
		$(elementId).click(function(){
		    ...
		    //SET TITLE MODAL
		    if(oid != 0){
			if(dataFor == 'showprovinceform'){
			    $(".addeditgeneral-title").html("Edit Province");
			}

		    }else{
			if(dataFor == 'showprovinceform'){
			    $(".addeditgeneral-title").html("Add Province");
			}
		    }
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxProvince", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxProvince", null, true, "json");
	    ...
	    //FORM HANDLER
	    chanelForm = function(){
		validateOptions("#<%= FrmPropinsi.fieldNames[FrmPropinsi.FRM_FIELD_ID_NEGARA] %>", 'text', 'has-error', 1, null);
		validateOptions("#<%= FrmPropinsi.fieldNames[FrmPropinsi.FRM_FIELD_KD_PROPINSI] %>", 'text', 'has-error', 1, null);
                validateOptions("#<%= FrmPropinsi.fieldNames[FrmPropinsi.FRM_FIELD_NM_PROPINSI] %>", 'text', 'has-error', 1, null);
		...
		dataTablesOptions("#provinceElement", "tableProvinceElement", "AjaxProvince", "listprovince", callBackDataTables);
	    }
	    ...
	    deletegeneral(".btndeleteProvince", ".<%= FrmPropinsi.fieldNames[FrmPropinsi.FRM_FIELD_ID_PROPINSI]%>");
		...
		if(generaldatafor == "showprovinceform"){
		    chanelForm();
		    onDone = function(data){
			runDataTables();
		    };
