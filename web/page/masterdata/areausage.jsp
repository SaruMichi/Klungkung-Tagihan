...
<%@page import="com.dimata.balismartisland.form.FrmMapAreaUsage"%>
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Di-SMARTCITY | Category</title>
  ...
  <body class="<%= skin %>">
	...
	<%@include file="../../template-component/sidebar-component.jsp" %>
      ...
      <div class="content-wrapper">
		...
		<div class='box box-primary'>
			...
			<button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showareausageform">
			    <i class="fa fa-plus"></i> Add Category
			</button>
                        <button class="btn btn-danger btndeleteareausage" data-for="areausage">
	...
	$(document).ready(function(){
	    ...
	    activeMenu("#masterdata", "#areausage");
		...
		$(elementId).click(function(){
		    ...
		    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxAreaUsage", ".addeditgeneral-body", false, "json");
		});
	    };
		...
		$(elementId).click(function(){
			...
			if(confirm(confirmText)){
			    ...
			    if(dataFor == "areausage"){
				onDone = function(data){
				    dataTablesOptions("#areaUsageElement", "tableAreaTypeElement", "AjaxAreaUsage", "listareausage", callBackDataTables);
				};
			    }
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxAreaUsage", null, true, "json");
		...
		addeditgeneral(".btneditgeneral");
                uploadicon(".btnuploadphoto");
                mapingForm(".btnmapping");
		iCheckBox();
	    }

	    //FORM HANDLER
	    areaUsageForm = function(){
		validateOptions("#<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_NAME] %>", 'text', 'has-error', 1, null);
		validateOptions("#<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_DETAIL] %>", 'text', 'has-error', 1, null);
	    }
		...
		dataTablesOptions("#areaUsageElement", "tableAreaTypeElement", "AjaxAreaUsage", "listareausage", callBackDataTables);
            ...
            //UPLOAD ICON
            uploadicon = function(elementId){
		$(elementId).click(function(){
                    $("#btnuploadicon").html("Save").removeAttr("disabled");
                    
		    $("#uploadphoto").modal("show");		    
		    oid = $(this).data('oid');
                    $("#formupload").attr("action","<%=approot%>/page/masterdata/uploadareausageimage.jsp?FRM_FIELD_OID="+oid+"&approot=<%=approot%>");
		    dataFor = $(this).data('for');
                    $("#tempname").val("");
		    $("#generaldatafor").val(dataFor);
		    $("#oiddata").val(oid);
            ...
            //MAPPING FORM
            mapingForm = function(elementId){
		$(elementId).click(function(){
                    $("#btnmappingsave").html("Save").removeAttr("disabled");
                    $("#btngeneralform").hide();
                    $("#btnmappingsave").show();
		    $("#addeditgeneral").modal("show");
		    command = $("#command").val();
		    ...
		    //SET TITLE MODAL
                    ...
                    if(dataFor == 'showmapareausageform'){
                        $(".addeditgeneral-title").html("Custom Data");
                    }
            ...
            saveMapping = function(elementId, checkboxelement){

		$(elementId).click(function(){
                    
		    dataFor = $(this).data("for");
                    oid = $("#<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_ID]%>").val();
		    var checkBoxes = (checkboxelement);
		    var vals = "";
		    $(checkboxelement).each(function(i){
			if($(this).is(":checked")){
		    ...
		    var confirmText = "Are you sure want to save these data?";
                    //alert(vals);
	    ...
	    deletegeneral(".btndeleteareausage", ".<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_ID]%>");
		    
	    runDataTables();
            uploadTrigger();
            saveMapping("#btnmappingsave",".<%= FrmMapAreaUsage.fieldNames[FrmMapAreaUsage.FRM_FIELD_CUSTOM_DATA_ID]%>");
		...
		if(generaldatafor == "showareausageform"){
		    areaUsageForm();
		    onDone = function(data){
                        //alert("test");
			runDataTables();
		...
		var color = "";
		for(var i = 0; i < <%= PstAreaUsage.statusList.length %>;i++){
		    if(color.length > 0){
			color +=","+$("#color"+i).val();
		    }else{
			color +=""+$("#color"+i).val();
		...
		$("#<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_STATUS_COLOR] %>").val(color);
	...
	<div class="modal-dialog nonprint">
		...
		<form id="generalform">
		    ...
		    <input type="hidden" name="<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_STATUS_COLOR] %>" id="<%= FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_STATUS_COLOR] %>">
