var guidely=(function(){
	...
	welcome:function(){
	    ...
	    this.showOverlay();
	    startBtn=$('<button>',{text:'Mulai Panduan',click:function(){that.close();that.start();}}).appendTo(controls);
	    noBtn=$('<button>',{text:'Tidak, Terima Kasih',click:function(){that.close();}}).appendTo(controls);
	    content.show();
	...
	next:function(){
	    ...
	    that=this;current=$('.guidely-guide:visible');
	...
	_setGuidePosition:function(number,content){
		...
		left=(offset.left-content.outerWidth())+30;content.addClass('guidely-anchor-right');
	    }else{
		left=(offset.left+number.outerWidth())-30;content.removeClass('guidely-anchor-right');
	    }
	...
	_createStartTrigger:function(){
	    var that=this;$('<a>',{
		'href':'javascript:;','class':'guidely-start-trigger','html':'Mulai Panduan','click':function(e){
		    e.preventDefault();
		    if(that._options.welcome){
	...
	_scrollToTop:function(callback){
	    ...
	    var target_top=target_offset.top;$('html, body').animate({
