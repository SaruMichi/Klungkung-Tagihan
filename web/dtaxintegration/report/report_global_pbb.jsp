<%-- 
    Document   : report_global_pbb
    Created on : Jul 20, 2015, 9:55:22 AM
    Author     : dimata005
--%>
...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPbbDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        ctrlist.addHeader("DENDA", "200px");//27
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+count);//1
            rowx.add(""+FRMHandler.userFormatStringDecimal(paymentPbb.getDendaSppt()));//2
...
<% 
    ...
    listPaymentPBB = PstPaymentPbb.listSum(0, 0, whereClause, "");
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                ...
                document.driver.action="report_global_pbb.jsp";
            ...
            function cmdPrintExcel(){
                var myWindow = window.open("report_global_pbb.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_pbb_global_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
                            ...
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <div class="box-header">
                                       ...
                                       <%if(userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_REPORT,AppObjInfo.G2_REPORT_PBB, AppObjInfo.OBJ_REPORT_GLOBAL_PBB, AppObjInfo.COMMAND_PRINT)){ %>        
                                           <div class="col-xs-2"><label>&nbsp;</label>  
