<%-- 
    Document   : ReportBusRevenueSummary
    Created on : May 1, 2015, 10:04:48 AM
    Author     : dimata005
--%>
<%@page import="com.dimata.travel.entity.report.ReportBusRevenue"%>
<%@page import="com.dimata.travel.session.SessBusRevenueSummary"%>
...
<%!//proses yang akan ditampilkan
    ...
    public String drawList(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ReportBusRevenue reportDetail = (ReportBusRevenue) objectClass.get(i);
            count=count+1;
            total=total+reportDetail.getTotal();
            rowx = new Vector(1, 1);
            rowx.add(""+count);
            rowx.add(""+reportDetail.getNoPolice());
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    listReportDetail = SessBusRevenueSummary.getListSummary(srcReportDetailSummary,0,0);
%>
<html>
    <head>
        <meta charset="UTF-8">
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                var myWindow = window.open("report_bus_revenue_summary_excel.jsp?tanggalStart=<%=startDate%>&tanggalEnd=<%=endDate%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdSearch(){
                ...
                document.driver.action="report_bus_revenue_summary.jsp";
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1>
                        Report
                        <small>Bus Revenue Summary</small>
                    </h1>
                    <!--<ol class="breadcrumb">
